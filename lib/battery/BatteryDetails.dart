...
class BatteryDetails extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title:  Text('Battery Info plugin '),
        ),
        body: Center(
          child: Column(
                    ...
                    if (snapshot.hasData) {
                      return Column(
                        children: [
                          ...
                          SizedBox(height: 20,),
                          Text("Battery Capacity: ${(snapshot.data!.batteryCapacity!/1000)} mAh"),
                          SizedBox(height: 20,),
                          Text("Technology: ${(snapshot.data!.technology)} "),
                          SizedBox(height: 20,),
                          Text("Battery present: ${snapshot.data!.present! ? "Yes" : "False"} "),
                          SizedBox(height: 20,),
                          Text("Scale: ${(snapshot.data!.scale)} "),
                          SizedBox(height: 20,),
                          Text("Remaining energy: ${-(snapshot.data!.remainingEnergy! * 1.0E-9)} Watt-hours,"),
                          SizedBox(height: 20,),
  ...
  Widget _getChargeTime(AndroidBatteryInfo data) {
    if (data.chargingStatus == ChargingStatus.Charging) {
      return data.chargeTimeRemaining == -1
          ...
          : Text("Charge time remaining: ${(data.chargeTimeRemaining! / 1000 / 60).truncate()} minutes");
    }
